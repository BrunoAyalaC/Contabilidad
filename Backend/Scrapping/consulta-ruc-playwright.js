...
async function consultaRucPlaywright(ruc) {
  ...
  try {
    ...
    // Recolectar texto de la página y de frames (si existen), luego parsear etiquetas conocidas
    ...
    try {
      ...
      for (let idx = 0; idx < snapFrames.length; idx++) {
        ...
        try {
          ...
          sections.push({ label: `frame[${idx}] ${url}`, frameUrl: url, text: text.trim(), html: html });
        } catch (e) {}
      }
    } catch (e) {}
          ...
          try {
            ...
            for (let j = 0; j < nowFrames.length; j++) {
              ...
              try {
                ...
                const fhtml = await ff.evaluate(() => document.documentElement.outerHTML).catch(() => '');
                snap.push({ frameIndex: j, frameUrl: furl, text: ftext.trim(), html: fhtml });
              } catch (e) {}
            }
